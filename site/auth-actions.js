export async function submitAuth(client, mode, values, origin, policyVersion) {
  ...
  if (mode === 'signup') {
    if (!values.terms)
      throw new Error('Accept the terms before creating an account.');
    ...
    return data.session
      ...
      : {
          message:
            'Check your inbox for an account confirmation email. If you already have an account, log in or reset your password.',
        };
  }
  if (mode === 'forgot') {
  ...
  if (mode === 'reset') {
    ...
    if (sessionError || !user)
      throw new Error(
        'Your reset link has expired. Request a new link to continue.',
      );
    const { error } = await client.auth.updateUser({ password });
    if (error) throw error;
    return {
      message:
        'Your password has been updated. You can now return to your account.',
      accountLink: true,
    };
  }
