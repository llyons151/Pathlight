...
test('login requires provider success before redirecting', async () => {
  ...
  await assert.rejects(
    submitAuth(
      { auth: { signInWithPassword: async () => ({ error: failure }) } },
      'login',
...
test('signup requires acceptance and sends policy metadata and fixed callback', async () => {
  ...
  const client = {
    auth: {
      signUp: async (values) => {
        called = true;
        assert.equal(
          values.options.emailRedirectTo,
  ...
  const result = await submitAuth(
    client,
    'signup',
    { email: 'u@example.com', password: 'long-test-password', terms: 'on' },
    'https://example.com',
    'v1',
  );
...
test('recovery gives a neutral response without disclosing account existence', async () => {
  const result = await submitAuth(
    {
      auth: {
        resetPasswordForEmail: async (_, options) => {
          assert.equal(
            options.redirectTo,
...
test('built routes and links work; sensitive source files cannot be served', async () => {
  ...
  try {
    ...
    assert.equal(
      (await fetch(origin + '/login/', { method: 'POST' })).status,
      405,
    );
    assert.equal(
      (await fetch(origin + '/login/')).headers.get('x-frame-options'),
      'DENY',
    );
    for (const path of [
      '/missing-page',
